    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">View and manage your weekly class schedule</p>
        </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-muted-foreground mb-2">No classes scheduled</p>
              <p className="text-sm text-muted-foreground">Your scheduled classes will appear here</p>
            </div>
          ) : (
              ...
              <div className="min-w-[900px]">
                <div className="grid grid-cols-8 border-b border-border">
                  ...
                  {weekDates.map((date, i) => (
                    ...
                    >
                      <p className={`text-sm font-medium ${isToday(date) ? "text-blue-600" : "text-muted-foreground"}`}>
                        {DAYS[i].slice(0, 3)}
                      </p>
                      <p className={`text-lg font-semibold ${isToday(date) ? "text-blue-600" : "text-foreground"}`}>
                        {date.getDate()}
                      </p>
                    </div>
                  ))}
          ...
          return (
            <Card key={i} className="border-border shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-foreground">{day}</CardTitle>
              </CardHeader>
              <CardContent>
                {dayClasses.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {dayClasses.map((cls) => (
                      ...
                      >
                        ...
                        <p className="text-xs text-muted-foreground">{cls.startTime.slice(0, 5)} • {cls.room}</p>
                      </div>
                    ))}
                  </div>
                )}
